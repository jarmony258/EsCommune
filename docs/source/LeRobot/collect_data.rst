...
- ``--dataset.episode_time_s=60`` 每次数据记录片段的持续时间（默认：**60秒**）。
- ``--dataset.reset_time_s=60`` 每轮结束后重置环境的持续时间（默认：**60秒**）。
- ``--dataset.num_episodes=50`` 总共要记录的片段数量（默认：**50**）。
...
- 录制过程中会自动创建检查点。
- 如果出现异常 (比如你本来要录50条，结果录到25条想去来一局，按ESC后停止录制), 或者你想在一个已经录制好50条数据的数据集里再加50条，可以通过使用 ``--resume=true`` 重新运行相同命令来恢复。恢复录制时，必须将 ``--dataset.num_episodes`` 设置为 **需要额外录制的剧集数量** ，而不是数据集中目标总剧集数！
